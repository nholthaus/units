// Case: `copysign` on a quantity measured from an ARBITRARY ORIGIN -- an affine reading (a datum) or a decibel LEVEL (a
...
int main()
{
	auto bad = units::copysign(units::temperature::celsius<double>(5.25), -1.0); // ill-formed
	(void)bad;
	return 0;
}
