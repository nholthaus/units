// Case: `hypot` on a quantity measured from an ARBITRARY ORIGIN -- an affine reading (a datum) or a decibel LEVEL (a
...
// expect-match: take hypot of differences
...
int main()
{
	auto bad = units::hypot(units::temperature::celsius<double>(3.25), units::temperature::celsius<double>(4.5)); // ill-formed
	(void)bad;
	return 0;
}
