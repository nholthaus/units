...
| `is_equivalent_conversion_factor<Cf1, Cf2>` / `is_equivalent_conversion_factor_v<Cf1, Cf2>` | `Cf1` and `Cf2` are equivalent conversion factors: the same dimension, conversion ratio, pi exponent and datum. (`is_equivalent_conversion_factor` in `include/units/core.h`) |
| `is_equivalent_unit<U1, U2>` / `is_equivalent_unit_v<U1, U2>` | `U1` and `U2` are equivalent units: the same conversion factor and the same numerical scale, so a value converts between them unchanged. The representation is not part of a unit's identity. (`is_equivalent_unit` in `include/units/core.h`) |
