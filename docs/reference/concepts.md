...
| `equivalent_unit<UnitTo, UnitFrom>` | two types which are **equivalent units** — the same conversion factor and numerical scale, so a value converts between them unchanged | `traits::is_equivalent_unit` (`equivalent_unit` in `include/units/core.h`) |
